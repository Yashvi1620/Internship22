...
module.exports.updatemodule=function(req,res){
    ...
    let endDate=req.body.endDate
    let utilizedHours=req.body.utilizedHours
    moduleModel.updateOne({_id:moduleid},{modulename:modulename,project:project,estimatedHours:estimatedHours,startDate:startDate,endDate:endDate,utilizedHours:utilizedHours},function(err,data){
        if(err){
            res.json({
                msg:"Something went wrong!!",
                status:-1,
